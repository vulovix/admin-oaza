...
export default function PublishArticle(): JSX.Element {
  ...
  const onCategoryChange = (items: OnChangeValue<{ value: string; label: string }, true>) => {
    onChange({
      target: {
        name: "categories",
  ...
  return (
    <div className="publish-article">
      ...
      <Input name="image" onChange={onChange} value={state?.image} labelComponent={() => <FormattedMessage id="Image URL" />} />
      <Select
